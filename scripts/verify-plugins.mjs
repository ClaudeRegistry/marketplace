...
for (const entry of marketplace.plugins) {
  ...
  for (const [id, title, fn] of CHECKS) {
    const r = entry && exists(pluginDir) ? fn(pluginDir, entry) : { status: 'fail', detail: 'plugin dir missing' };
...
fs.writeFileSync(path.join(ROOT, '.claude-plugin', 'verified.json'), JSON.stringify(result, null, 2) + '\n');
